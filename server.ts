  ...
  app.post('/api/questions/generate', requireUser, async (request, response) => {
    try {
      ...
      const roundInstructions: Record<string, string> = {
        'concept-mcq': 'Each question must be a Concept MCQ with exactly four labeled options in the question text and one clearly identified correct option in the answer.',
        'fill-in-the-blank': 'Each question must be a fill-in-the-blank sentence or short paragraph with exactly one meaningful blank shown as ____.',
        scenario: 'Each question must be a realistic workplace or interview scenario that asks the student to reason through decisions, tradeoffs, or debugging steps.',
        architecture: 'Each question must ask for architecture, system design, scaling, tradeoff, component, reliability, or data-flow reasoning.',
        'coding-round': 'Each question must be a coding problem with constraints, expected input/output behavior, and an answer that explains an efficient approach and provides code or pseudocode.',
        'mock-interview': 'Each question must be an open-ended mock interview prompt with an answer that models a strong spoken response.',
        'faang-tagged': 'Each question must be FAANG-style: concise, high-signal, practical, and include an answer with reasoning and common follow-up expectations.',
      };
      const systemPrompt = [
        'You are a strict domain classifier and interview question generator.',
        'First classify whether the requested topic genuinely belongs to the selected domain. Be strict, but allow normal subtopics of that domain.',
        'If the topic does not belong, return JSON with domainMismatch true, a short reason, and an empty questions array.',
        'If the topic belongs, return domainMismatch false and exactly 30 numbered, high-quality questions.',
        'Every question must strictly match the selected round type. Do not mix MCQ, scenario, architecture, coding, mock, and fill-blank formats.',
        'Questions must be specific to the topic, not generic interview advice. Answers must be technically correct, detailed, and useful for interview prep.',
        'Do not use placeholders, repeated questions, vague answers, markdown tables, or markdown code fences. JSON strings may contain short code snippets when the round type is coding.',
      ...
      const userPrompt = [
        ...
        `Round format rule: ${roundInstructions[roundType]}`,
        'Generate exactly 30 distinct questions and answers for students preparing for interviews.',
        'Each answer should be 3-8 sentences unless the selected round type needs code or structured steps.',
      ].join('\n');
        ...
        (payload) => {
          ...
          const questions = Array.isArray(source.questions)
            ? source.questions.map((item) => {
              ...
              if (item.question.length < 20 || item.answer.length < 40) return false;
              if (/^(question|answer|n\/a|todo|placeholder)$/i.test(item.question)) return false;
              return true;
            }).slice(0, 30)
            : [];
          return {
