...
import { ArrowRight, ChartPie, CheckCircle2, Github, Heart, LoaderCircle, Play, Search, TrendingUp, X } from 'lucide-react';
...
function ReadinessTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-xl border border-blueprint-line bg-card px-3 py-2 text-sm text-primary shadow-xl">
      {name ? <p className="font-semibold text-primary">{name}</p> : null}
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-[1320px] flex-col gap-6 px-4 pb-14 pt-6 sm:px-6 lg:px-10">
        ...
        <section className="surface-card">
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_220px]">
            <div className="surface-inset flex flex-col justify-center gap-3">
              {readinessChartData.map((entry) => (
                <div key={entry.name} className="flex items-start gap-3">
                  <span className="mt-1 h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: entry.fill }} />
                  <div>
                    <p className="text-ui-label font-semibold text-primary">{entry.name}</p>
                    <p className="text-body-md text-blueprint-muted">{entry.value ? `${Math.round(entry.value)}% contribution to your readiness score` : 'No data yet — complete sessions to see this'}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="surface-inset h-[180px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={readinessChartData} dataKey="value" nameKey="name" innerRadius={42} outerRadius={70} paddingAngle={3}>
                    {readinessChartData.map((entry) => <Cell key={entry.name} fill={entry.fill} />)}
                  </Pie>
        ...
        <section className="surface-card">
          ...
          <div className="relative mt-5 grid gap-1" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(18px, 1fr))' }}>
            {displayedHeatmapDays.map((day) => (
              <span
                key={day.key}
                onMouseEnter={(e) => { setHoveredActivity(day); setTooltipPos({ x: e.clientX, y: e.clientY }); }}
                onMouseMove={(e) => setTooltipPos({ x: e.clientX, y: e.clientY })}
                onMouseLeave={() => { setHoveredActivity(null); setTooltipPos(null); }}
                onFocus={(e) => { setHoveredActivity(day); setTooltipPos({ x: e.currentTarget.getBoundingClientRect().left, y: e.currentTarget.getBoundingClientRect().top }); }}
                onBlur={() => { setHoveredActivity(null); setTooltipPos(null); }}
                tabIndex={0}
            ...
            {hoveredActivity && tooltipPos ? (
              <div className="pointer-events-none fixed z-50 rounded-xl border border-blueprint-line bg-card px-3 py-2 text-xs text-primary shadow-xl" style={{ left: tooltipPos.x + 12, top: tooltipPos.y - 40 }}>
                ...
                {domainStats ? (
                  <div className="mt-3 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-body-md text-primary font-semibold">{domainStats.overallReadiness}% readiness</p>
                    </div>
                    <p className="text-body-md text-blueprint-muted">{domainStats.totalAttempts} total attempt{domainStats.totalAttempts === 1 ? '' : 's'} tracked</p>
                    {domainStats.strongestDomain ? (
                      <p className="text-body-md text-blueprint-muted">Strongest: {domainStats.strongestDomain.label}</p>
                    ) : null}
                  </div>
                ) : (
                  <p className="mt-2 text-body-md text-blueprint-muted">Complete sessions to see your stats.</p>
                )}
              </div>
            </div>
          </button>
        ...
        <footer className="flex flex-col items-center gap-3 border-t border-blueprint-line py-6 text-sm text-blueprint-muted text-center">
          <p className="flex items-center gap-1">
            &copy; 2026 Repoid. All rights reserved.
          </p>
          <p className="flex items-center gap-2">
            Made by{' '}
            <a href="https://www.linkedin.com/in/satwikchandra45/" target="_blank" rel="noopener noreferrer" className="font-semibold text-primary hover:underline">Satwik</a>
